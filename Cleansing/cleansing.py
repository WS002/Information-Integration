...
def _fkcm(str1, str2):
    set1 = set([word.lower() for word in re.findall("\w+", str1)])
    set2 = set([word.lower() for word in re.findall("\w+", str2)])
        ...
        if index+windowSize < len(data):
            ...
            for i, secondItem in enumerate(data[(index+1):(index+windowSize)]):
                ...
                if ( (item[1][2] is not None and item[1][3] is not None) or (secondItem[1][2] is not None and secondItem[1][3] is not None) ):
                    ...
                    if item[1][6] and secondItem[1][6]:
                        long = item[1][6]
                        short = secondItem[1][6]
                        if len(long) < len(short):
                            long, short = short, long
                        fkcmValue = _fkcm(short, long)

                    if fkcmValue >= 0 and fkcmValue < 0.2:
                        pass # no duplicate
                    elif euDistance > -1 and levenshteinDistance > -1:
...
def _merge(duplicates):
    ...
    for item in duplicates:
        ...
        # test, if description matches
        if firstItem[6] and secondItem[6]:  # Use the truth value of strings / None
            long = firstItem[6]
            short = secondItem[6]
            if len(long) < len(short):
                long, short = short, long

            containment = _fkcm(short, long)
            if containment > 0.5:
                values.append(long)
                valueString += ', %s'
                keys += ', description'
            elif firstItem[2]:  # first item has coordinates
                values.append(firstItem[6])
                valueString += ', %s'
                keys += ', description'
            else:  # second item has coordinates (per definition)
                values.append(secondItem[6])
                valueString += ', %s'
                keys += ', description'
        elif firstItem[6] != '' and firstItem[6] is not None:
            values.append(firstItem[6])
            valueString += ', %s'
            keys += ', description'
        ...
        print "Unite", firstItem
        print "With ", secondItem
